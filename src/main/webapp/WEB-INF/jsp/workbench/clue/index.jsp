	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			let loadTable = function(){
				$("#clue_create_form :input").val("");

				// 进入页面 加载时从application上下文中取得
				// 字典值 并且回显 而owner可能要经常维护 故从数据库中取
				$.get("/user/queryAll", function (result) {
					$(result).each(function () {
						$("#select_owner").append('<option value="'+this.id+'">'+this.name+'</option>');
					})
					// 默认所有者为当前的登录用户
					$("#select_owner").val("${user.id}");
				}, "json");
				// 查询所有的记录
				$.get("/clue/queryAll", function (result) {
					let htmlArr = [];
					$(result).each(function (index) {
						htmlArr.push('<tr class="'+(index % 2 === 0 ? "active" : "")+'">\
										<td><input name="id" value="'+this.id+'" type="checkbox" /></td>\
										<td><a clueId="'+ this.id +'" style="text-decoration: none; cursor: pointer;">'+this.fullname+this.appellation+'</a></td>\
										<td>'+this.company+'</td>\
										<td>'+this.phone+'</td>\
										<td>'+this.mphone+'</td>\
										<td>'+this.source+'</td>\
					...
					$("#clue_table_body").html(htmlArr.join(""));
				})
			};
			...
			$("#save_button").click(function () {
				$.post("/clue/insertOrUpdate", $("#clue_create_form").formJSON(), function (result) {
					if (result.success) {
			...
			// 点击线索记录跳转到详情页
			$("#clue_table_body").on("click", "a[clueId]", function () {
				// console.log("/clue/detail.html?clueId="+$(this).attr("clueId"))
				location = "/clue/detail.html?clueId="+$(this).attr("clueId");
			})
	...
	<div class="modal-dialog" role="document" style="width: 90%;">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="clue_create_form"  class="form-horizontal" role="form">
					<!--隐藏域-->
					<input name="id" type="hidden">
					<div class="form-group">
						<label for="select_owner" class="col-sm-2 control-label">所有者<span style="font-size: 15px; color: red;">*</span></label>
						<div class="col-sm-10" style="width: 300px;">
							<select name="owner" class="form-control" id="select_owner">
					...
					<div class="form-group">
						...
						<div class="col-sm-10" style="width: 300px;">
							<select name="appellation" class="form-control" id="create-call">
								<option value="">--请选择--</option>
								<c:forEach items="${appellationList}" var="val">
					...
					<div class="form-group">
						...
						<div class="col-sm-10" style="width: 300px;">
							<select name="state" class="form-control" id="create-status">
								<option value="">--请选择--</option>
								<c:forEach items="${clueStateList}" var="val">
									<option value="${val.value}">${val.text}</option>
								</c:forEach>
							</select>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10" style="width: 300px;">
							<select name="source" class="form-control" id="create-source">
								<option value="">--请选择--</option>
								<c:forEach items="${sourceList}" var="val">
									<option value="${val.value}">${val.text}</option>
								</c:forEach>
							</select>
						</div>
						...
						<div class="col-sm-10" style="width: 300px;">
							<input name="nextcontacttime" time type="text" class="form-control" id="create-nextContactTime">
						</div>
					</div>
	...
	<div style="width: 100%; position: absolute;top: 5px;">
		<div class="btn-toolbar" role="toolbar" style="height: 80px;">
			<form class="form-inline" role="form" style="position: relative;top: 8%;left: 5px; ">
				...
				<button type="submit" style="float: none;" class="btn btn-default">查询</button>

			</form>
		</div>
		<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;top: 40px;">
			<div class="btn-group" style="position: relative; top: 18%;">
				<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#create_modal"><span class="glyphicon glyphicon-plus"></span> 创建</button>
